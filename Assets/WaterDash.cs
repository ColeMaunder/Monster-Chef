...
public class WaterDash : MonoBehaviour
{
    ...
    void OnCollisionStay2D(Collision2D hit)
    {
        ...
        if (hit.gameObject.tag == "Water")
        {
            ...
            Vector3 inDirect = (hit.transform.position - player.transform.position).normalized; //which direction moving in from
            ...
            */
            ...
            {
                Physics2D.IgnoreCollision(hitColider, player.GetComponent<Collider2D>());
                playerRB.linearVelocity = inDirect * - waterDashSpeed;
                print("leap");
    ...
    void OnCollisionExit2D(Collision2D hit) {
        ...
        {
            Collider2D hitColider = hit.gameObject.GetComponent<TilemapCollider2D>();
            Physics2D.IgnoreCollision(hitColider, player.GetComponent<Collider2D>(), false);
